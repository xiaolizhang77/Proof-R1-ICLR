...
class vLLMHttpServer:
    ...
    def __init__(
        ...
        self._loaded_lora_path = VLLM_LORA_PATH
        ...
        if self.lora_as_adapter:
            ...
            if lora_loaded:
                lora_request = LoRARequest(
                    lora_name=VLLM_LORA_NAME, lora_int_id=VLLM_LORA_INT_ID, lora_path=self._loaded_lora_path
                )
    ...
    async def load_lora_adapter(self, lora_path: str, global_steps: int | None = None):
        ...
        if not os.path.isfile(adapter_model):
            raise FileNotFoundError(f"LoRA adapter weights not found: {adapter_model}")
        if os.path.getsize(adapter_model) <= 40:
            raise RuntimeError(f"LoRA adapter weights look empty: {adapter_model}")
        ...
        loaded = await self.engine.add_lora(lora_request)
        if not loaded:
