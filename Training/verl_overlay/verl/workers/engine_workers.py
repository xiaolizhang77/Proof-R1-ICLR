...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    async def update_weights(self, global_steps: int = None, mode: str = "auto"):
        ...
        defer_weight_resume = (
            ...
            and getattr(self, "base_sync_done", False)
        )
        weights_resumed = False
        if self.config.rollout.free_cache_engine and not defer_weight_resume:
            await self.rollout.resume(tags=["weights"])
            weights_resumed = True
        ...
        if self.config.rollout.free_cache_engine and not weights_resumed:
            await self.rollout.resume(tags=["weights"])
            weights_resumed = True
    ...
    async def load_lora_adapter(self, lora_path: str, global_steps: int = None):
        ...
        assert "rollout" in self.role, "load_lora_adapter only supports rollout role"

        set_expandable_segments(False)
        try:
            log_gpu_memory_usage("Before load_lora_adapter resume weights", logger=logger)

            if self.config.rollout.free_cache_engine:
                await self.rollout.resume(tags=["weights"])
            log_gpu_memory_usage("After load_lora_adapter resume weights", logger=logger)

            await self.rollout.load_lora_adapter(lora_path=lora_path, global_steps=global_steps)

            if self.config.rollout.free_cache_engine:
                await self.rollout.resume(tags=["kv_cache"])
            log_gpu_memory_usage("After load_lora_adapter resume kv_cache", logger=logger)
